...
void OpenCLMatrixMultiply(Matrix *input0, Matrix *input1, Matrix *result)
{
    ...
    device_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
                         sizeof(int) * result->shape[0] * result->shape[1], 
                         NULL, &err);
    ...
    size_t local_size[2] = {16, 16};  // 16x16 thread block
    size_t global_size[2] = {
    ((result->shape[0] + local_size[0] - 1) / local_size[0]) * local_size[0],
    ((result->shape[1] + local_size[1] - 1) / local_size[1]) * local_size[1]
};
