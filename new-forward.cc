    ...
    const int B, const int M, const int C, const int H, const int W, const int K) 
{
    ...
    *device_x_unroll = clCreateBuffer(this->opencl->context, CL_MEM_READ_WRITE, x_unroll_size, NULL, &err);
    ...
    const int C, const int H, const int W, const int K) 
{
    ...
    err = clSetKernelArg(this->opencl->kernel, 0, sizeof(cl_mem), &device_x_unroll);
    err |= clSetKernelArg(this->opencl->kernel, 1, sizeof(cl_mem), &device_x);
    err |= clSetKernelArg(this->opencl->kernel, 2, sizeof(int), &B);
    err |= clSetKernelArg(this->opencl->kernel, 3, sizeof(int), &C);
    err |= clSetKernelArg(this->opencl->kernel, 4, sizeof(int), &H);
    err |= clSetKernelArg(this->opencl->kernel, 5, sizeof(int), &W);
    err |= clSetKernelArg(this->opencl->kernel, 6, sizeof(int), &K);
    CHECK_ERR(err, "clSetKernelArg kernel_im2col");

    // Launch im2col kernel
    err = clEnqueueNDRangeKernel(this->opencl->queue,this->opencl->kernel, 3, NULL, global_size_im2col, local_size_im2col, 0, NULL, NULL);
