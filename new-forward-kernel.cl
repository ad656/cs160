...
__kernel void im2col(__global float *unrolled, __global float *x, const int B,
    ...
    const int H_unroll = C_in * K * K;  // Height of unrolled matrix (# of rows)
    const int W_unroll = H_out * W_out;  // Width of unrolled matrix (# of columns)
    ...
    const int bc_idx = get_global_id(2);  // Combined batch and channel index
        ...
        for (int mask_offset_row = 0; mask_offset_row < K; mask_offset_row++) {
            for (int mask_offset_col = 0; mask_offset_col < K; mask_offset_col++) {
                ...
                if (row_o_in_bounds && col_o_in_bounds) {
                    // Calculate unrolled matrix indices
                    // Column in unrolled matrix corresponds to output position
                    int col_u = row_o * W_out + col_o;
                    
                    // Row in unrolled matrix corresponds to input channel and kernel position
                    // Organized as: C_in * K * K total rows, with K*K consecutive rows per channel
